...
const createsession = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    //  
    for (let entry of empDetail) {
     const found =  await addReportServiceInstance.find({pmr_pm_code:addReport.pmr_pm_code,pmr_nbr:nbr,pmr_employe:entry.pmr_employe})
      if(found)
        {console.log('existe')}
      else{  
          await addReportServiceInstance.create({
              ...entry,
              pmr_nbr: nbr,
              pmr_start_date:addReport.pmr_start_date,
              pmr_end_date:addReport.pmr_end_date,
              pmr_site:addReport.pmr_site,
              pmr_duration:addReport.pmr_duration,
              pmr_domain: user_domain,
              pmr_pm_code: addReport.pmr_pm_code,
              pmr_inst: addReport.pmr_inst,
              pmr_task: addReport.pmr_task,
