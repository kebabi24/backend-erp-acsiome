...
const getSpecificationsCodes = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        return res
            .status(200)
            .json({ message: "found specifications", data: specifications})
    } catch (e) {
        logger.error("🔥 error: %o", e)
        return next(e)
    }
...
const findOneSpecificationWithDetails = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        return res
            .status(200)
            .json({ message: "found one specification", data: {specification ,specificationDetails}  })
    } catch (e) {
        logger.error("🔥 error: %o", e)
        return next(e)
    }
...
const createTestsHistory = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        return res
            .status(201)
            .json({ message: "created succesfully", data: { createdTestsHistory  } })
    } catch (e) {
        logger.error("🔥 error: %o", e)
        return next(e)
    }
