import LocationFilterService from "../../services/location-filter"
...
const{user_domain} = req.headers
    ...
    logger.debug("Calling Create location Filter endpoint")
    try {
...
const findOne = async (req: Request, res: Response, next: NextFunction) => {
    const logger = Container.get("logger")
    logger.debug("Calling find one  location Filter endpoint")
    try {
        const locationServiceInstance = Container.get(LocationFilterService)
        const {id} = req.params
...
const findAll = async (req: Request, res: Response, next: NextFunction) => {
    const logger = Container.get("logger")
    logger.debug("Calling find all location Filter endpoint")
    try {
        const locationServiceInstance = Container.get(LocationFilterService)
