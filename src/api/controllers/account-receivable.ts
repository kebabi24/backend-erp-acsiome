...
const findInvBetweenDate = async (req: Request, res: Response, next: NextFunction) => {
    const logger = Container.get("logger")
    logger.debug("Calling find by  all account endpoint")
    const{user_code} = req.headers 
    const{user_domain} = req.headers 
 ...
 let result = []
        ...
        const AccountReceivableServiceInstance = Container.get(AccountReceivableService)
        const customerServiceInstance = Container.get(CustomerService)
        const arb = await AccountReceivableServiceInstance.findInvswithadress({
              ar_type:'I',ar_effdate: { [Op.between]: [req.body.date, req.body.date1]},
