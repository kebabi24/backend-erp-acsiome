...
const create = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const tk = await taskServiceInstance.create({
      ...Task,
      tk_domain: user_domain,
    ...
    return res.status(201).json({ message: 'created succesfully', data: tk });
  } catch (e) {
    //#
...
const findAll = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'fetched succesfully', data: tasks });
  } catch (e) {
    logger.error('🔥 error: %o', e);
    return next(e);
  }
...
const update = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'fetched succesfully', data: tk });
  } catch (e) {
    logger.error('🔥 error: %o', e);
    return next(e);
  }
...
const findAllwithDetails = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const pos = await sequelize.query(
      'SELECT  PUBLIC.tk_mstr.id as "tid"  , *  FROM   PUBLIC.tk_mstr,  PUBLIC.tkd_det  where PUBLIC.tkd_det.tkd_domain = ? and PUBLIC.tK_mstr.tk_domain = PUBLIC.tkd_det.tKd_domain  and  PUBLIC.tkd_det.tKd_code = PUBLIC.tK_mstr.tk_code  ORDER BY PUBLIC.tk_mstr.id ASC',
