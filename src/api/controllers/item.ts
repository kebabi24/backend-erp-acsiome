...
const findByOneStk = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all item endpoint');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
...
const findOneFile = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find one  code endpoint');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
  ...
  const  filepath = uploadsPath +  item.pt_drwg_loc
...
const findAllItemswithstk = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const ihs =await Sequelize.query("SELECT  PUBLIC.pt_mstr.id as id , PUBLIC.pt_mstr.pt_part , PUBLIC.pt_mstr.pt_desc1 ,PUBLIC.pt_mstr.pt_drwg_loc, PUBLIC.pt_mstr.pt_um, PUBLIC.pt_mstr.pt_ord_mult,PUBLIC.pt_mstr.pt_pur_lead,PUBLIC.pt_mstr.pt_phantom, PUBLIC.pt_mstr.pt_site,PUBLIC.pt_mstr.pt_loc, PUBLIC.pt_mstr.pt_taxable,PUBLIC.pt_mstr.pt_taxc,PUBLIC.pt_mstr.pt_price,PUBLIC.pt_mstr.pt_pur_price,PUBLIC.tx2_mstr.tx2_tax_pct , (PUBLIC.ld_det.ld_qty_oh - PUBLIC.ld_det.ld_qty_all) as qty FROM PUBLIC.pt_mstr  , PUBLIC.tx2_mstr, PUBLIC.ld_det where PUBLIC.tx2_mstr.tx2_tax_code = PUBLIC.pt_mstr.pt_taxc and PUBLIC.ld_det.ld_part = PUBLIC.pt_mstr.pt_part  and (PUBLIC.ld_det.ld_qty_oh - PUBLIC.ld_det.ld_qty_all ) > 0 and PUBLIC.pt_mstr.pt_domain = ?  ORDER BY qty DESC", { replacements: [user_domain], type: QueryTypes.SELECT });
 console.log(ihs)
...
const it = {
 ...
 // pt_drwg_loc = jsonData['pt_drwg_loc'],
  ...
  pt_salepacking : jsonData['pt_salepacking'],
    ...
    if (req.file) {
      console.log("here1")
   item = await itemServiceInstance.update(
      {   ...it,
        pt_drwg_loc: req.file.originalname,
