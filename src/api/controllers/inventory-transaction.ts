...
const findByreport = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
  try {
    ...
    const inventoryTransactionServiceInstance = Container.get(InventoryTransactionService);
    const emps = await itemserviceinstance.find({pt_domain:user_domain,pt_prod_line:'EPI' });
    let result = []
    for (let emp of emps){
        // const blouse = await inventoryTransactionServiceInstance.findSpecial({
        //   where: {
...
id:emp.id,
pt_part:emp.pt_part,
pt_desc1:emp.pt_desc1,
pt_part_type:emp.pt_part_type,
pt_draw:emp.pt_draw,
pt_group:emp.pt_group,
pt_model:emp.pt_model,
pt_rev:emp.pt_rev,
pt_break_cat:emp.pt_break_cat,
qty_oh:0,
rctpo:0,
issepi:0,
retepi:0,
...
const findByExs = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
  try {
    const {date1,date2,time1,time2} = req.body
    ...
    const trs = await inventoryTransactionServiceInstance.findSpecial({
      ...
      group: ['tr_effdate', 'tr_lot','tr_qty_loc', 'tr_ref','tr_type','tr_site', 'tr_loc','createdAt','tr_part'],
      raw: true,
    });
...
const inv = await inventoryTransactionServiceInstance.find({
      ...
      { 
        ...
        result.push({
          id: i,
          tr_type:inv[inv.length-1].tr_type,
          tr_line:inv[inv.length-1].tr_line,
          tr_part:inv[inv.length-1].tr_part,
          tr_desc:inv[inv.length-1].tr_desc,
          tr_effdate: tr.tr_effdate,
          ...
          tr_user1:inv[inv.length-1].tr_user1,
          tr_user2:inv[inv.length-1].tr_user2,
          tr_qty_loc: inv[inv.length-1].tr_qty_loc,
          tr_ref: tr.tr_ref,
          tr_nbr:inv[inv.length-1].tr_nbr,
          tr_um:inv[inv.length-1].tr_um,
          tr_um_conv:inv[inv.length-1].tr_um_conv,
    ...
    for (const data of detail) {
      ...
      if (ld)
        ...
        await locationDetailServiceInstance.create({
          ...
          dec01:new_price,
          dec02:new_price * Number(item.tr_qty_loc) * Number(item.tr_um_conv),
          created_by: user_code,
              created_ip_adr: req.headers.origin,
              last_modified_by: user_code,
              last_modified_ip_adr: req.headers.origin,
        ...
        await locationDetailServiceInstance.update(
          {
            ...
            chr04:it.tr_addr,
             dec01:ld.dec01,
          dec02:ld.dec01 * (Number(ld.ld_qty_oh) - Number(item.tr_qty_loc) * Number(item.tr_um_conv)),
            last_modified_by: user_code,
            last_modified_ip_adr: req.headers.origin,
          },
          { id: ld.id },
...
const rctEpi = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
...
const issEpi = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
    ...
    for (const item of detail) {
      ...
      if (ld1)
        await locationDetailServiceInstance.update(
          ...
          dec02:ld1.dec01 * Number(ld1.ld_qty_oh) + Number(item.tr_qty_loc) * Number(item.tr_um_conv),
            last_modified_by: user_code,
            last_modified_ip_adr: req.headers.origin,
          },
          { id: ld1.id },
        ...
        await locationDetailServiceInstance.create({
          ...
          ld_rev:pt.pt_rev,
          dec01:ld.dec01,
          dec02:ld.dec01 * Number(item.tr_qty_loc) * Number(item.tr_um_conv),
        });
      }
      await inventoryTransactionServiceInstance.create({
        ...item,
    ...
    for (const data of detail) {
      ...
      if (ld)
        ...
        await locationDetailServiceInstance.create({
          ...
          dec02:sct.sct_mtl_tl * Number(item.tr_qty_loc) * Number(item.tr_um_conv),
          created_by: user_code,
              created_ip_adr: req.headers.origin,
              last_modified_by: user_code,
              last_modified_ip_adr: req.headers.origin,
...
const rctWoIss = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
  ...
  logger.debug('Calling update one  code endpoint');
  try {
    const { detail, it,woddetail,wod } = req.body;
    ...
    for (const item of woddetail) {
      ...
      if (ld)
       { 
        ...
        const wo = await workOrderServiceInstance.findOne({ id: wod.tr_lot });
        let routing : any;
        let emp: any;
        let locbegin : any;
        let chr01:any;
        ...
        {  await inventoryTransactionServiceInstance.create({
         ...
         tr_addr:routing,
         tr_user1:emp,
         tr_gl_date: wod.tr_effdate,
         tr_qty_loc: -1 * Number(item.tr_qty_loc),
         tr_qty_chg: -1 * Number(item.tr_qty_loc),
         tr_loc_begin: Number(locbegin),
         tr_type: 'ISS-WO',
         tr_date: new Date(),
         tr_price: sct.sct_mtl_tl,
         tr_mtl_std: sct.sct_mtl_tl,
        ...
        tr__chr02:pt.pt_break_cat,
        tr__chr03:pt.pt_group,
        dec01:Number(new Date(wod.tr_effdate).getFullYear()),
        dec02:Number(new Date(wod.tr_effdate).getMonth() + 1),
        tr_program:new Date().toLocaleTimeString(),
        tr_batch:chr01,
        tr_grade:grade,
         tr_gl_amt: Number(item.tr_qty_loc) * Number(item.tr_um_conv) * Number(item.tr_price),
    ...
    for (const item of detail) {
      ...
      if (remain.tag_cnt_qty !== undefined) {
        ...
        if (remain.ld_rev != 'M') {
          ...
          dec02:sctdet.sct_mtl_tl * Number(qty),
              created_by: user_code,
              created_ip_adr: req.headers.origin,
              last_modified_by: user_code,
              last_modified_ip_adr: req.headers.origin,
...
const findtrDate = async (req: Request, res: Response, next: NextFunction) => {
  ...
  //     dec01:trs.dec01,
  //     dec02:trs.dec02,
  //     tr_so_job:trs.tr_so_job,
  //     last_modified_by:trs.last_modified_by
  //   })
  //   i = i + 1
  // }
  // }
...
const retissWo = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
    ...
    for (const item of detail) {
      ...
      if (wo)
        await locationDetailServiceInstance.create({
          ...
          ld_loc: item.tr_loc,
          ld_status: item.tr_status,
          ld_qty_oh: Number(item.tr_qty_loc) * Number(item.tr_um_conv) * -1,
          ld_expire: item.tr_expire,
          ld__log01: status.is_nettable,
          ld_domain:user_domain,
          ld_grade: ld.ld_grade,
          ld__chr01:ld.ld__chr01,
          chr01:pt.pt_draw,
          chr02:pt.pt_break_cat,
          chr03:pt.pt_group,
          int01:pt.int01,
          ...
          ld_rev:pt.pt_rev,
           dec01:sct.sct_mtl_tl,
          dec02:sct.sct_mtl_tl * (Number(item.tr_qty_loc) * Number(item.tr_um_conv) * -1),
          created_by: user_code,
              created_ip_adr: req.headers.origin,
              last_modified_by: user_code,
              last_modified_ip_adr: req.headers.origin,
        ...
        dec01:Number(new Date(it.tr_effdate).getFullYear()),
        dec02:Number(new Date(it.tr_effdate).getMonth() + 1),
        tr_batch:ld.ld_batch,
        tr_grade:ld.ld__chr01,
         tr_gl_amt: Number(item.tr_qty_loc) * Number(item.tr_um_conv) * Number(item.tr_price),
         created_by: user_code,
         created_ip_adr: req.headers.origin,
         last_modified_by: user_code,
...
const reprint = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
...
const findByGroupEmp = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const {user_domain} = req.headers;
  try {
...
const findByGroupEpi = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const {user_domain} = req.headers;
  try {
    ...
    for (const data of detail) {
      ...
      if (ld){
        ...
        await locationDetailServiceInstance.create({
          ...
          ld_rev:pt.pt_rev,
           dec01:sctdet.sct_mtl_tl,
          dec02:sctdet.sct_mtl_tl * Number(item.tr_qty_loc) * Number(item.tr_um_conv),
          created_by: user_code,
              created_ip_adr: req.headers.origin,
              last_modified_by: user_code,
              last_modified_ip_adr: req.headers.origin,
...
const findByIss = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
...
const findByEpi = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
...
const findByEmp = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling find by  all code endpoint');
  const{user_code} = req.headers 
  const{user_domain} = req.headers
...
const issTrPal = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  logger.debug('Calling update one  code endpoint');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
