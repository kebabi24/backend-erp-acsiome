...
const createImmob = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
...
const createPlq = async (req: Request, res: Response, next: NextFunction) => {
  const logger = Container.get('logger');
  const { user_code } = req.headers;
  const { user_domain } = req.headers;
...
let datelr = `${days}/${months}/${years}`;
   ...
   // console.log(item)
          str += `${req.body.ps.psh_nbr}|${req.body.pshnbr}|${datelr}|${req.body.ps.psh_cust}|${item.psh_site}|${item.psh_loc}|${item.psh_part}|${item.psh_serial}|${item.psh_qty_ship}|\n`;
