...
const getTemplate = (name : string) => {
	...
	switch (name)
	{
        ...
        case 'prh' : return template.body_prh;
        case 'it-tr' : return template.body_itTr;
        case 'it-unp' : return template.body_it;
        case 'rct-unp' : return template.body_it;
	}
}
...
const getHeader = (name) => {
	...
	switch (name)
	{
        ...
        case 'prh' : return template.header_prh;
        case 'it-tr' : return template.header_itTr;
        case 'it-unp' : return template.header_it;
        case 'rct-unp' : return template.header_it;
	}
}
...
const getData = (rawData, name : string) => {
   // const sub = name.split('-')[0];
    switch (name)
    {
        ...
        case 'prh' : return prep.preparePrhData(rawData);
        case 'it-tr' : return prep.prepareItTrData(rawData);
        case 'it-unp' : return prep.prepareItData(rawData, name);
        case 'rct-unp' : return prep.prepareItData(rawData, name);
    }
}
...
const getHeaderSize = (name : string) => {
   //const sub = name.split('-')[0];
    switch (name)
    {
        case 'it-tr' : return "470px";
        case 'it-unp' : return "470px";
        case 'rct-unp' : return "470px";
